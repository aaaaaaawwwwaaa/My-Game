...
const TYPE_ART = {
  Orc:['#355a32','#85b85b','#161d13'], Deamon:['#601d1d','#df633d','#1e0808'], Order:['#35506f','#d7eaff','#cba646'], Chaos:['#3e225a','#a66ad4','#170d20'], Forsaken:['#334c40','#85a77f','#151b17'], 'Soul Bound':['#4d243a','#d16aa8','#170b13'], 'Glacial Guardian':['#28677b','#b8f4ff','#102128'], Enlightened:['#6d5c2f','#e3ca78','#24200f']
};
...
function pixelPortrait(c){
  const t=(c.types||[])[0]||'Order', pal=TYPE_ART[t]||TYPE_ART.Order, h=hashName(c.name); const eye=(h%3)+1;
  const horn=(c.types||[]).includes('Deamon')||c.kind==='Boss'; const crown=c.kind==='Boss';
  return `<svg viewBox="0 0 96 64" preserveAspectRatio="none" aria-label="Pixel portrait of ${c.name}">
    <rect width="96" height="64" fill="${pal[2]}"/><rect y="44" width="96" height="20" fill="${pal[0]}" opacity=".55"/>
    <rect x="30" y="16" width="36" height="32" fill="${pal[0]}"/><rect x="34" y="12" width="28" height="8" fill="${pal[0]}"/>
    ${horn?'<rect x="25" y="8" width="8" height="13" fill="'+pal[1]+'"/><rect x="63" y="8" width="8" height="13" fill="'+pal[1]+'"/>':''}
    ${crown?'<rect x="35" y="4" width="26" height="8" fill="#e5b94a"/><rect x="36" y="0" width="5" height="7" fill="#f5d87b"/><rect x="46" y="0" width="5" height="7" fill="#f5d87b"/><rect x="56" y="0" width="5" height="7" fill="#f5d87b"/>':''}
    <rect x="38" y="26" width="6" height="5" fill="${pal[1]}"/><rect x="52" y="26" width="6" height="5" fill="${pal[1]}"/>
    <rect x="46" y="35" width="${4+eye}" height="3" fill="#1a1110"/><rect x="24" y="47" width="48" height="17" fill="${pal[0]}"/>
    <rect x="18" y="51" width="12" height="13" fill="${pal[1]}" opacity=".75"/><rect x="66" y="51" width="12" height="13" fill="${pal[1]}" opacity=".75"/>
    <text x="48" y="60" text-anchor="middle" font-family="monospace" font-size="6" font-weight="bold" fill="#fff" opacity=".85">${c.name.slice(0,14)}</text>
  </svg>`
}
...
async function aiPlacement(){
  ...
  let plays=0;while(plays<3){const idx=p.hand.findIndex(c=>c.kind!=='Spell');if(idx<0)break;const open=p.board.map((x,i)=>!x?i:-1).filter(i=>i>=0);if(!open.length)break;open.sort((a,b)=>{const av=human.board[a]?3:0,bv=human.board[b]?3:0;return bv-av||Math.abs(2-a)-Math.abs(2-b)});const lane=open[0],c=p.hand.splice(idx,1)[0];p.board[lane]=instantiate(c,1);log(`Abyss AI deploys ${c.name} to lane ${lane+1}.`,'system');plays++;render();await sleep(330)}
